...
//~ using namespace RMT;
	...
	void apply_ising_Top_inhom(itpp::cvec& state,double ising, double Jinhom) { // {{{
		double* dev_R;
		double* dev_I;
		int l=state.size();
		...
		itppcuda::itpp2cuda(state,&dev_R,&dev_I);
			...
			for(int Position2=1+Position1; Position2<nqubits; Position2++){
				...
				Ui_kernel<<<numblocks,numthreads>>>(Position1,Position2,dev_R,dev_I,cos(ising),sin(ising),l);
...
apply_ising_Top_inhom(state, J.getValue()+Jpert.getValue(), J.getValue()+Jinhompert.getValue()+Jpert.getValue());

apply_inhomogeneous_kick(state, bpert, binhom);

apply_ising_Top_inhom(staterev, J.getValue()-Jpert.getValue(), J.getValue()-Jinhompert.getValue()-Jpert.getValue());
